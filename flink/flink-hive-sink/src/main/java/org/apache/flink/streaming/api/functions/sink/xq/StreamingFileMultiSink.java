 ...
 * StreamingFileSink#forRowFormat(Path, Encoder)} or { StreamingFileSink#forBulkFormat(Path,
 * BulkWriter.Factory)}.
 *
 ...
 */
@PublicEvolving
public class StreamingFileMultiSink<IN> extends RichSinkFunction<IN>
...
//    private final BucketsBuilder<IN, ?, ? extends BucketsBuilder<IN, ?, ?>> bucketsBuilder;
    private final List<BucketsBuilder> bucketsBuilderList;

    // --------------------------- runtime fields -----------------------------

    private transient StreamingFileSinkHelper<IN> helper;
    private transient Map<String,StreamingFileSinkHelper<IN>> helperMap;
    ...
    public StreamingFileMultiSink(List<BucketsBuilder> bucketsBuilderList, long bucketCheckInterval) {

        Preconditions.checkArgument(bucketCheckInterval > 0L);

        this.bucketsBuilderList = Preconditions.checkNotNull(bucketsBuilderList);
        this.bucketCheckInterval = bucketCheckInterval;
    }
    ...
    /**
     * Creates the builder for a {@link StreamingFileMultiSink} with row-encoding format.
     *
     ...
     *     configured. In order to instantiate the sink, call {RowFormatBuilder#build()} after
     *     specifying the desired parameters.
     */
    public static <IN> DefaultRowFormatBuilder<IN> forRowFormat(
            final Path basePath, final Encoder<IN> encoder,String tableName) {
        return new DefaultRowFormatBuilder<>(basePath, encoder, new DateTimeBucketAssigner<>(),tableName);
    }

    /**
     * Creates the builder for a {@link StreamingFileMultiSink} with bulk-encoding format.
     *
     * @param basePath the base path where all the buckets are going to be created as
     *     sub-directories.
     ...
     * @param <IN> the type of incoming elements
     * @return The builder where the remaining of the configuration parameters for the sink can be
     *     configured. In order to instantiate the sink, call { BulkFormatBuilder#build()}
     *     after specifying the desired parameters.
     */
    public static <IN> DefaultBulkFormatBuilder<IN> forBulkFormat(
            final Path basePath, final BulkWriter.Factory<IN> writerFactory,String tableName) {
        return new DefaultBulkFormatBuilder<>(
                basePath, writerFactory, new DateTimeBucketAssigner<>(),tableName);
    }
    ...
    public abstract static class BucketsBuilder<
        ...
        @Internal
        public abstract String getTableName() throws IOException;
    }
    ...
    public static class RowFormatBuilder<IN, BucketID, T extends RowFormatBuilder<IN, BucketID, T>>
        ...
        protected RowFormatBuilder(
                Path basePath, Encoder<IN> encoder, BucketAssigner<IN, BucketID> bucketAssigner,String tableName) {
            this(
                    basePath,
                    encoder,
                    bucketAssigner,
        ...
        public <ID>
                RowFormatBuilder<IN, ID, ? extends RowFormatBuilder<IN, ID, ?>>
...
//        }
    ...
    /**
     * Builder for the vanilla {@link StreamingFileMultiSink} using a row format.
     *
    ...
    public static final class DefaultRowFormatBuilder<IN>
        ...
        private DefaultRowFormatBuilder(
                Path basePath, Encoder<IN> encoder, BucketAssigner<IN, String> bucketAssigner,String tableName) {
            super(basePath, encoder, bucketAssigner,tableName);
        }
    }
    ...
    public static class BulkFormatBuilder<
        ...
        protected BulkFormatBuilder(
                ...
                BucketAssigner<IN, BucketID> assigner,String tableName) {
            this(
                    basePath,
                    writerFactory,
                    assigner,
        ...
        public <ID>
                BulkFormatBuilder<IN, ID, ? extends BulkFormatBuilder<IN, ID, ?>>
...
//        }
    ...
    /**
     * Builder for the vanilla {@link StreamingFileMultiSink} using a bulk format.
     *
     * @param <IN> record type
     */
    public static final class DefaultBulkFormatBuilder<IN>
        ...
        private DefaultBulkFormatBuilder(
                Path basePath,
                BulkWriter.Factory<IN> writerFactory,
                BucketAssigner<IN, String> assigner,String tableName) {
            super(basePath, writerFactory, assigner,tableName);
        }
    }
    ...
    public void initializeState(FunctionInitializationContext context) throws Exception {
        Map<String, Buckets<IN, ?>> bucketsMap = new HashMap<>();
        int indexOfThisSubtask = getRuntimeContext().getIndexOfThisSubtask();
        for (BucketsBuilder<IN, ?, ? extends BucketsBuilder<IN, ?, ?>> bucketsBuilder : bucketsBuilderList) {
            bucketsMap.put(bucketsBuilder.getTableName(), bucketsBuilder.createBuckets(indexOfThisSubtask));
        }
        this.helper =
