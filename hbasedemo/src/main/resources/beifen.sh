...
function statusCheck() {
    ...
    hbase hbck 1>${Shell_Path}/log/check/log.txt 2>&1
    echo "集群状态："
    echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    sed -n '/Number of live region servers/,/Number of regions in transition/p' ${Shell_Path}/log/check/log.txt
    echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    echo "正常的表："
    echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    sed -n '/Summary:/,/Status:/p' ${Shell_Path}/log/check/log.txt | grep 'is okay' | cut -d " " -f 2
    echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"

    local nu=`grep 'inconsistencies detected' ${Shell_Path}/log/check/log.txt | cut -d " " -f 1`
    if [ ${nu} != 0 ];then
      echo "检测出有"$nu"个表不一致，请查看日志文件："${Shell_Path}/log/check/log.txt
    else
      echo "没有不一致的表"
    fi
}
...
function snapshot() {
    ...
    echo "" > ${Shell_Path}/log/snapshot/log.txt
