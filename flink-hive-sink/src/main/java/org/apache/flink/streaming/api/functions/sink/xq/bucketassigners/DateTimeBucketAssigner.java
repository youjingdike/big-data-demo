...
package org.apache.flink.streaming.api.functions.sink.xq.bucketassigners;
...
public class DateTimeBucketAssigner<IN> implements BucketAssigner<IN, String> {
	...
	public String getBucketId(IN element, BucketAssigner.Context context) {
		String[] bucketStr = new String[2];
		if (dateTimeFormatter == null) {
			dateTimeFormatter = DateTimeFormatter.ofPattern(formatString).withZone(zoneId);
		}
		bucketStr[0] = bucketStr[1] = dateTimeFormatter.format(Instant.ofEpochMilli(context.currentProcessingTime()));
		return bucketStr[0];
	}
