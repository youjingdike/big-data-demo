...
/**
 * The manager of the different active buckets in the {@link StreamingFileMultiSink}.
 *
 * <p>This class is responsible for all bucket-related operations and the actual {@link
 * StreamingFileMultiSink} is just plugging in the functionality offered by this class to the lifecycle
 * of the operator.
 *
...
public class Buckets<IN, BucketID> {
    ...
    public void initializeState(
            final List<BucketState> bucketStates, final Long partCounterState)
            throws Exception {
    ...
    private void initializePartCounter(final Long partCounterState) throws Exception {
//        long maxCounter = 0L;
//        for (long partCounter : partCounterState.get()) {
//            maxCounter = Math.max(partCounterState, maxCounter);
//        }
//        maxPartCounter = maxCounter;
        if (partCounterState == null) {
            maxPartCounter = 0L;
        } else {
            maxPartCounter = Math.max(partCounterState, 0L);
        }
    }

    private void initializeActiveBuckets(final List<BucketState> bucketStates) throws Exception {
...
//        }
        if (bucketStates!=null) {
            for (BucketState bucketState : bucketStates) {
    ...
    public void snapshotState(
            ...
            final List<BucketStateInfo> bucketStateInfoList,
            final List<MaxPartCounterStateInfo> maxPartCounterStateInfoList)
            throws Exception {
...
//        partCounterStateContainer.add(maxPartCounter);
        ...
        snapshotActiveBuckets(checkpointId, bucketStateInfoList);

        MaxPartCounterStateInfo maxPartCounterStateInfo = new MaxPartCounterStateInfo(tableName,maxPartCounter);
        maxPartCounterStateInfoList.add(maxPartCounterStateInfo);
    }

    private void snapshotActiveBuckets(
            final long checkpointId, final List<BucketStateInfo> bucketStateInfoList)
            throws Exception {
        BucketStateInfo bucketStateInfo = new BucketStateInfo(tableName);
...
//                            bucketStateSerializer, bucketState);
            ...
            bucketStateInfo.getStateList().add(bucketState);

            if (LOG.isDebugEnabled()) {
                LOG.debug("Subtask {} checkpointing: {}", subtaskIndex, bucketState);
            }
        }
        bucketStateInfoList.add(bucketStateInfo);
    }
